function [smeared_intensity, E_range] = ARPES_simulation_Expert(eigenbands, SurfaceOrb, Ef, sigma, gamma, fidelity, temperature, photonE, polarization, IncidentTheta, IncidentPhi, PolarizationAngle)
% Inputs:
...
if ischar(polarization) && strcmp(polarization, 'unpolarized')
    ...
    for e = 1:2
        ...
        intensity_sum = intensity_sum + weighted_projection(SurfaceOrb, weights, target_orbs, M);
    end
    ...
    weights = get_weights(polarization, photonE, orb_names);
    intensity = weighted_projection(SurfaceOrb, weights, target_orbs, M);
end
...
function I = weighted_projection(orb_proj, weights, target_orbs, M)
...
for k = 1:Nk
    for b = 1:Nb
        ...
        for a = 1:Natom
            for o = target_orbs
                val = val + orb_proj(k,b,a,o) * weights(o)* M(o);
            end
        end
        I(k,b) = abs(val)^2;
    end
...
% Computes the Faddeeva function using MATLAB built-in
    w = exp(-z.^2) .* erfc(real(-1i*z));  % Accurate and vectorized
end