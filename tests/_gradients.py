"""Provide program-wide differentiability checks.

Extended Summary
----------------
Every differentiability test calls
``assert_gradients_match_finite_differences`` or ``assert_grad_matches_fd``
from this module. Ad-hoc finite differences are a review-blocking defect.
A harness failure on a physics function indicates a physics failure. The
gradients construct the Fisher matrix. A false zero in the Jacobian removes
...
def _central_leaf_grad(
    ...
    real part.
    """
    leaf: Shaped[Array, "..."] = jnp.asarray(leaves[leaf_index])
    steps: Float64[Array, "..."] = fd_step(
        jnp.real(leaf), scale_floor=scale_floor
    )
    flat_leaf: Shaped[Array, " n_parameter"] = jnp.ravel(leaf)
    flat_steps: Float64[Array, " n_parameter"] = jnp.ravel(steps)
    basis: Shaped[Array, "n_parameter n_parameter"] = jnp.eye(
        flat_leaf.size, dtype=flat_leaf.dtype
    )

    def evaluate(
        delta: Shaped[Array, " n_parameter"],
    ) -> Float64[Array, ""]:
        perturbed_leaves: List[Shaped[Array, "..."]] = list(leaves)
    ...
    real_deltas: Shaped[Array, "n_parameter n_parameter"] = (
        basis * flat_steps[:, None]
    )
    real_gradient: Float64[Array, "n_parameter n_parameter"] = jax.vmap(
        lambda delta: (evaluate(delta) - evaluate(-delta)) / (2.0 * flat_steps)
    )(real_deltas)
    real_diagonal: Float64[Array, " n_parameter"] = jnp.diag(real_gradient)
    if not jnp.issubdtype(leaf.dtype, jnp.complexfloating):
        gradient: Shaped[Array, "..."] = jnp.reshape(real_diagonal, leaf.shape)
        return gradient

    imaginary_deltas: Shaped[Array, "n_parameter n_parameter"] = (
        1j * basis * flat_steps[:, None]
    )
    imaginary_gradient: Float64[Array, "n_parameter n_parameter"] = jax.vmap(
        lambda delta: (evaluate(delta) - evaluate(-delta)) / (2.0 * flat_steps)
    )(imaginary_deltas)
    imaginary_diagonal: Float64[Array, " n_parameter"] = jnp.diag(
        imaginary_gradient
    )
...
def central_fd_grad(
    ...
    parameter. This cost restricts the helper to compact verification models.
    """
    leaves: List[Any]
    treedef: PyTreeDef
    leaves, treedef = jax.tree_util.tree_flatten(theta)
    array_leaves: List[Shaped[Array, "..."]] = [
        jnp.asarray(leaf) for leaf in leaves
    ]
    jitted_fn: ScalarLoss = jax.jit(fn)
    gradient_leaves: List[Shaped[Array, "..."]] = [
        _central_leaf_grad(
...
def assert_grad_matches_fd(
    ...
    *,
    ...
    """
    step_leaves: List[Float64[Array, "..."]] = [
    ...
    for (path, actual), expected, step in zip(
        ...
        tolerance: Float64[Array, "..."] = (
            absolute_tolerance + relative_tolerance * jnp.abs(expected)
        )
        difference: Float64[Array, "..."] = jnp.abs(actual - expected)
...
def assert_nonzero_grad(
    ...
    *,
    ...
    The helper checks every leaf by default. ``sensitive_paths`` selects exact
    JAX key-path strings. By default, require each selected leaf to exceed
    ...
    path_leaves: List[Tuple[Tuple[object, ...], Shaped[Array, "..."]]]
        ...
        if path_name in selected_paths:
            if elementwise:
                ...
                if magnitudes.size == 0:
                    ...
                    raise AssertionError(message)
                insensitive: Shaped[Array, " n_parameter"] = (
                    magnitudes <= min_norm
                )
...
@jaxtyped(typechecker=beartype)
def assert_gradients_match_finite_differences(
    fn: ScalarLoss,
    theta: PyTree,
    *,
    regime: GradRegime = "smooth",
    ...
    **kwargs: Any,
    ...
    """Run finite, finite-difference, and sensitivity checks together.

    Differentiability tests use this single entry point. The helper
...
def complex_step_derivative(
    fn: Callable[[Complex128[Array, "..."]], Complex128[Array, "..."]],
    x: Float64[Array, "..."],
    *,
    ...
    complex_value: Complex128[Array, "..."] = fn(
        x.astype(jnp.complex128) + 1j * h * resolved_direction
    )
    imaginary_part: Float64[Array, "..."] = jnp.imag(complex_value)
