...
def test_g16_sto_and_hydrogenic_envelope_values_gradients_and_tails() -> None:
    ...
    final_state: FinalStateSpec = make_final_state_spec()
    sto_reference: np.ndarray = np.asarray(
        radial_bvals(
            sto,
    ...
    hydrogenic_reference: np.ndarray = np.asarray(
        radial_bvals(
            hydrogenic,
            jnp.asarray(tail_momenta),
            reference_profile,
...
def test_g16_near_condition_limit_cancellation_battery() -> None:
    ...
    exponent_values: np.ndarray = np.asarray(exponents[0])
    coefficient_values: np.ndarray = np.asarray(coefficients[0])
    primitive_norms: np.ndarray = (2.0 * exponent_values) ** (
        effective_principal + 0.5
    ) / math.sqrt(float(gamma(2.0 * effective_principal + 1.0)))
    overlap: np.ndarray = (
