...
class TestPlotArpesSpectrum(chex.TestCase):
    """Validate :func:`diffpes.plots.plot_arpes_spectrum`.
    ...
    :see: :func:`~diffpes.plots.plot_arpes_spectrum`
    """
...
class TestApplyKpathTicks(chex.TestCase):
    """Validate :func:`diffpes.plots.apply_kpath_ticks`.
    ...
    :see: :func:`~diffpes.plots.apply_kpath_ticks`
    """
...
class TestPlotMomentumMap(chex.TestCase):
    """Validate :func:`diffpes.plots.plot_momentum_map`.

    :see: :func:`~diffpes.plots.plot_momentum_map`
    """
...
class TestSpectrumExtent(chex.TestCase):
    """Validate :func:`~diffpes.plots.spectrum_extent`.

    Covers the extent tuple values and the acceptance of JAX and
    NumPy axes.

    :see: :func:`~diffpes.plots.spectrum_extent`
    """

    def test_extent_reads_axis_ends(self) -> None:
        """Compute the extent from the first and last axis elements.

        The test passes a momentum axis from -0.5 to 0.5 and an energy
        axis from -2.0 to 1.0. It expects the tuple
        ``(-0.5, 0.5, -2.0, 1.0)``.
        ...
        assertions.
        """
        momentum_axis: Float64[Array, " 5"]
        energy_axis: Float64[Array, " 7"]
        extent: Tuple[float, float, float, float]

        momentum_axis = jnp.linspace(-0.5, 0.5, 5)
        energy_axis = jnp.linspace(-2.0, 1.0, 7)
        extent = spectrum_extent(momentum_axis, energy_axis)
        chex.assert_equal(extent, (-0.5, 0.5, -2.0, 1.0))

    def test_numpy_axes_give_host_floats(self) -> None:
        """Accept NumPy axes and return host floats.

        The test passes NumPy float64 axes. It expects a tuple of four
        Python floats with the axis end values.
        ...
        assertions.
        """
        momentum_axis: Float64[NDArray, " 3"]
        energy_axis: Float64[NDArray, " 4"]
        extent: Tuple[float, float, float, float]
        value: float

        momentum_axis = np.linspace(0.0, 1.0, 3)
        energy_axis = np.linspace(-1.0, 0.0, 4)
        extent = spectrum_extent(momentum_axis, energy_axis)
        for value in extent:
            chex.assert_equal(isinstance(value, float), True)
        chex.assert_equal(extent, (0.0, 1.0, -1.0, 0.0))


class TestPlotSpectralCut(chex.TestCase):
    """Validate :func:`~diffpes.plots.plot_spectral_cut`.

    Covers the physical extent, the image orientation, the Fermi and
    guide lines, the log transform, and axis reuse.

    :see: :func:`~diffpes.plots.plot_spectral_cut`
    """

    def test_extent_orientation_and_fermi_line(self) -> None:
        """Plot the cut with a physical extent and one Fermi line.

        The test plots a 5 by 4 intensity map. It expects the
        transposed image shape ``(4, 5)``, the physical extent, and one
        horizontal guide line at zero energy.
        ...
        """
        momentum_axis: Float64[Array, " 5"]
        energy_axis: Float64[Array, " 4"]
        intensity: Float64[Array, "5 4"]
        fig: Figure
        ax: Axes
        image: AxesImage
        extent: Tuple[float, float, float, float]

        momentum_axis = jnp.linspace(-0.4, 0.4, 5)
        energy_axis = jnp.linspace(-2.0, 0.5, 4)
        intensity = jnp.ones((5, 4), dtype=jnp.float64)
        fig, ax, image = plot_spectral_cut(
            intensity, momentum_axis, energy_axis
        )
        chex.assert_equal(image.get_array().shape, (4, 5))
        extent = tuple(float(value) for value in image.get_extent())
        chex.assert_equal(extent, (-0.4, 0.4, -2.0, 0.5))
        chex.assert_equal(len(ax.lines), 1)
        chex.assert_equal(
            tuple(float(v) for v in np.asarray(ax.lines[0].get_ydata())),
            (0.0, 0.0),
        )
        plt.close(fig)

    def test_guides_log_counts_and_axis_reuse(self) -> None:
        """Plot guide lines and log counts on a caller-supplied axis.

        The test requests one momentum guide, one energy guide, and
        ``log_counts`` without a Fermi line. It expects two guide
        lines, the ``log1p`` image data, and the supplied axis.
        ...
        momentum_axis = jnp.linspace(-0.4, 0.4, 5)
        energy_axis = jnp.linspace(-2.0, 0.5, 4)
        intensity = 3.0 * jnp.ones((5, 4), dtype=jnp.float64)
        fig, ax = plt.subplots()
        out_fig, out_ax, image = plot_spectral_cut(
            intensity,
            momentum_axis,
            energy_axis,
            ax=ax,
        ...
        chex.assert_trees_all_close(
            float(image.get_array().max()), float(jnp.log1p(3.0))
        )
        plt.close(fig)
