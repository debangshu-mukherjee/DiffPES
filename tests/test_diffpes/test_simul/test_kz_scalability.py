"""Validate the inert Plan-08b scalability evidence.
...
S1-kz/S2-kz allocations and companions without compiling the target.
"""
...
_ARTIFACT_PATH: Path = _REFERENCE_DIRECTORY / "08b_kz_scalability.json"
_CONSUMER_MANIFEST_PATH: Path = (
    _REFERENCE_DIRECTORY / "08b_kz_scalability_manifest.json"
)
_HARNESS_PATH: Path = (
    _REPOSITORY_ROOT
    / "diffpes-plans"
    / "verification"
    / "measure_08b_kz_scalability.py"
)
_PLANS_ARTIFACT_PATH: Path = (
    _REPOSITORY_ROOT
    / "diffpes-plans"
    / "verification"
    / "08b_kz_scalability.json"
)
_PLANS_MANIFEST_PATH: Path = (
    _REPOSITORY_ROOT
    / "diffpes-plans"
    / "verification"
    / "08b_kz_scalability_manifest.json"
)
...
_CONSUMER_MANIFEST_SHA256: str = (
    "d122c1a96e0edd6de44b156c76bfb1b8f3a61de553aca19f961a4ee52719d292"
)
_HARNESS_SHA256: str = (
    "c93b148145933c71e862d4a1d0f0be3c296703d6afb0d3c276d569c1917cfd93"
)
_PLANS_MANIFEST_SHA256: str = (
    "c6c884f4542b6c30abca846ed5096f33181a06090a3295409e8323e5e8e10778"
)
...
def test_kz_scalability_authenticates_authority_mirror_and_graph() -> None:
    ...
    Notes
    -----
    The test imports neither JAX nor the mutable plans-side harness.
    """
    ...
    assert _sha256(_HARNESS_PATH) == _HARNESS_SHA256
    ...
    assert consumer_manifest["artifact"] == {
        "path_from_diffpes_root": (
            "tests/test_diffpes/_reference_data/08b_kz_scalability.json"
        ),
        "sha256": _ARTIFACT_SHA256,
    }
    assert consumer_manifest["plans_artifact"] == {
        "path": "diffpes-plans/verification/08b_kz_scalability.json",
        "sha256": _ARTIFACT_SHA256,
    }
    assert consumer_manifest["generator"] == {
        "mutable_authority_count": 1,
        "path": "diffpes-plans/verification/measure_08b_kz_scalability.py",
    ...
    assert plans_manifest["schema_version"] == 1
    assert plans_manifest["artifact"]["sha256"] == _ARTIFACT_SHA256
    assert plans_manifest["generator"]["sha256"] == _HARNESS_SHA256
    assert "sole mutable authority" in plans_manifest["authority"]
    ...
    g6: Dict[str, Any] = artifact["g6_authority"]
    assert artifact["g6_selected_n_kz"] == 2048
    assert g6["selected_n_kz"] == 2048
    authority_name: str
    for authority_name in ("artifact", "manifest", "generator"):
        authority: Dict[str, str] = g6[authority_name]
        assert (
            _sha256(_REPOSITORY_ROOT / authority["path"])
            == authority["sha256"]
        )


def test_kz_scalability_freezes_literal_shape_and_allocation_gates() -> None:
    ...
    Notes
    -----
    The literal target is compile-only; XLA ``memory_analysis`` is authoritative.
    """
    artifact: Dict[str, Any] = _load_json(_ARTIFACT_PATH)
    assert artifact["schema_version"] == 1
    assert artifact["schema"] == "org.diffpes.verification.kz-scalability.v1"
    assert artifact["gate_ids"] == ["08b.S1-kz", "08b.S2-kz"]
...
def test_kz_scalability_freezes_remat_and_photon_scan_companions() -> None:
    ...
    Notes
    -----
    Subtract the required scan output, photon input, gradient output, and scalar
    output before applying the 64 KiB auxiliary-memory flatness gate.
    """
    artifact: Dict[str, Any] = _load_json(_ARTIFACT_PATH)
    ...
    scan: Dict[str, Any] = artifact["s2_hv_auxiliary_memory"]
