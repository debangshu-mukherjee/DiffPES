"""Verify independent escape-depth double-counting evidence.

The tests consume only a frozen inert JSON record. They bind its bytes to the
sole external NumPy/SciPy generator and inspect the preregistered materiality
...
_ARTIFACT_PATH: Path = (
    _REFERENCE_DIRECTORY / "kz_double_counting_materiality.json"
)
_CONSUMER_MANIFEST_PATH: Path = (
    _REFERENCE_DIRECTORY / "kz_double_counting_materiality_manifest.json"
)
_ARTIFACT_SHA256: str = (
    "033149757f4b9a72f7fedf8ab7da2bac6504d43a74082b3771176b30e6dc251b"
)
_CONSUMER_MANIFEST_SHA256: str = (
    "4ccff30831718c8c98696682b56f21790392ed040f13c7e1c054225141de533e"
)
_GENERATOR_SHA256: str = (
    "68d502e5d3fee0723f3892981af6e3280bf5ce9c2d01bbe9feb5daeca082546f"
)
...
def test_materiality_artifact_authenticates_authority_and_values() -> None:
    """Bind every inert byte to one external numeric authority.

    The test hashes the artifact and consumer manifest. It also mutates one
    in-memory artifact byte as a tamper control.

    Notes
    -----
    Authenticate the generator identity from inert metadata. Never import or
    execute the external generator inside pytest.
    """
    ...
    assert consumer_manifest["artifact"] == {
        "path_from_diffpes_root": (
            ...
            "kz_double_counting_materiality.json"
        ),
        "sha256": _ARTIFACT_SHA256,
    }
    assert consumer_manifest["authority_artifact"] == {
        "name": "kz_double_counting_materiality.json",
        "sha256": _ARTIFACT_SHA256,
    }
    assert consumer_manifest["generator"] == {
        "mutable_authority_count": 1,
        "name": "generate_kz_double_counting_materiality.py",
...
def test_materiality_artifact_freezes_the_registered_battery() -> None:
    ...
    assert artifact["artifact_id"] == "kz-double-counting-materiality-v1"
    assert artifact["status"] == "green"
    assert artifact["requirements"] == [
        "exclusive-escape-depth-routes",
        "finite-window-mass-conservation",
    ]
