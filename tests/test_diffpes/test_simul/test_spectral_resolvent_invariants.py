"""Certify analytic, gauge, and derivative resolvent invariants.
...
def _quadratic_peak_and_hwhm(
    ...
    returned: Tuple[float, float] = peak_position, hwhm
    return returned


class TestResolventDifferentiability:
    """Pin generic source, energy, self-energy, and regulator derivatives.

    The case compares automatic derivatives for every generic coordinate with
...
class TestResolventGaugeAndSumRules:
    """Pin exact-degeneracy gauge invariance and normalized spectral mass.

    The cases rotate degenerate states, integrate the full-line density, refine
    the quadrature, and test the regulator-limit acceptance criteria.
    """
        ...
        def masses(order: int) -> Float64[Array, " 5"]:
            ...
            def one(
                ...
                returned: Float64[Array, ""] = jnp.sum(
                    quadrature * intensity * jacobian
                )
                return returned

            returned: Float64[Array, " 5"] = jax.vmap(one)(
                centers, widths, projected_weights
            )
            return returned
    ...
    def test_regulator_limit_acceptance_battery(self) -> None:
        """Enforce every frozen regulator mass, width, peak, and limit check.
