...
from arpyes.tb.diagonalize import (
    diagonalize_single_k,
    diagonalize_tb,
    vasp_to_diagonalized,
)
from arpyes.tb.hamiltonian import make_1d_chain_model, make_graphene_model
...
class TestDiagonalizeSingleK:
    ...
    def test_eigenvalues_real(self):
        ...
        H = jnp.array([[1.0, 0.5 + 0.1j], [0.5 - 0.1j, 2.0]], dtype=jnp.complex128)
...
class TestDiagonalizeTB:
    ...
    def test_eigenvalues_sorted(self):
        ...
        model = make_graphene_model()
        kpoints = jnp.array([[0.0, 0.0, 0.0], [0.1, 0.2, 0.0], [0.3, 0.1, 0.0]])
        diag = diagonalize_tb(model, kpoints)
        for i in range(3):
            assert float(diag.eigenvalues[i, 0]) <= float(diag.eigenvalues[i, 1])
