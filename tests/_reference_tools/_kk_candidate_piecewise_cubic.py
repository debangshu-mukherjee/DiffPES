"""Cell-integrated piecewise-cubic core PV transform.
...
DESCRIPTION = (
    "Analytic cell PV integration of cubic interpolants on four-node stencils, "
...
def core_pv_transform(
    ...
    Each cell ``[x_i, x_{i+1}]`` integrates the cubic through the four-node
    stencil starting at ``clip(i-1, 0, n_kk-4)``: interior cells use the
    centered stencil around the cell, and the first and last cells clamp to
    the one-sided edge stencils.  Every stencil contains both cell endpoints,
    so adjacent interpolants agree exactly at the shared node values.
    ...
    Q2*(u2**2-u1**2)/2 + Q3*(u2**3-u1**3)/3``.  Log terms are regrouped by
    grid node before evaluation, exposing the exact cancellation at interior
    nodes.
    """
    ...
    n_kk = grid.shape[0]
    if n_kk < 4:
        msg = "piecewise-cubic PV transform requires at least four grid nodes"
        raise ValueError(msg)

    h = grid[1] - grid[0]
    cell_indices = jnp.arange(n_kk - 1)
    stencil_starts = jnp.clip(cell_indices - 1, 0, n_kk - 4)

    y0 = values[stencil_starts]
    y1 = values[stencil_starts + 1]
    y2 = values[stencil_starts + 2]
    y3 = values[stencil_starts + 3]
    x0 = grid[stencil_starts]

    # C(w) = y0 + linear*(w-x0) + quadratic*(w-x0)^2 + cubic*(w-x0)^3.
    linear = (-11.0 * y0 + 18.0 * y1 - 9.0 * y2 + 2.0 * y3) / (6.0 * h)
    quadratic = (2.0 * y0 - 5.0 * y1 + 4.0 * y2 - y3) / (2.0 * h**2)
    cubic = (-y0 + 3.0 * y1 - 3.0 * y2 + y3) / (6.0 * h**3)
    ...
    q2 = quadratic[None, :] + 3.0 * cubic[None, :] * offset
