...
def plot_arpes_spectrum(
    ...
    :see: :class:`~.test_arpes_maps.TestPlotArpesSpectrum`
...
def apply_kpath_ticks(
    ...
    :see: :class:`~.test_arpes_maps.TestApplyKpathTicks`
...
def plot_arpes_with_kpath(  # noqa: PLR0913, PLR0917
    ...
    :see: :class:`~.test_arpes_maps.TestPlotArpesWithKpath`
...
def plot_momentum_map(  # noqa: PLR0913, PLR0917
    ...
    :see: :class:`~.test_arpes_maps.TestPlotMomentumMap`
...
def spectrum_extent(
    ...
    The function converts the two physical axes into the four-value
    ``extent`` tuple that ``matplotlib.axes.Axes.imshow`` expects for an
    energy-momentum image with ``origin="lower"``.

    :see: :class:`~.test_arpes_maps.TestSpectrumExtent`

    Parameters
    ----------
    momentum_axis : Float64[Array, " n_k"] | Float64[NDArray, " n_k"]
        Momentum axis in 1/Angstrom.
    energy_axis : Float64[Array, " n_e"] | Float64[NDArray, " n_e"]
        Energy axis in eV.
    ...
    extent : Tuple[float, float, float, float]
        The tuple ``(k_min, k_max, e_min, e_max)`` as host floats.

    Notes
    -----
    Reads the first and last element of each axis and casts each value to
    a host ``float``. The function assumes monotonic axes and does not
    reorder them.
    """
...
@jaxtyped(typechecker=beartype)
def plot_spectral_cut(  # noqa: PLR0913, PLR0917
    ...
    momentum_guides: Optional[Tuple[float, ...]] = None,
    energy_guides: Optional[Tuple[float, ...]] = None,
    guide_color: str = "white",
    xlabel: str = r"$k$ ($\mathrm{\AA}^{-1}$)",
    ylabel: str = r"$E - E_F$ (eV)",
    title: str = "",
) -> Tuple[Union[Figure, SubFigure], Axes, AxesImage]:
    r"""Plot an energy-momentum intensity map on physical axes.

    The function renders one spectral cut with
    ``matplotlib.axes.Axes.imshow``. The horizontal axis carries the
    physical momentum coordinate, and the vertical axis carries the
    energy relative to the Fermi level. The function accepts an existing
    axis. It creates a figure and axis when the caller supplies none.

    :see: :class:`~.test_arpes_maps.TestPlotSpectralCut`
    ...
    1. **Normalize the plotting arrays**::
       ...
       ``log_counts`` the values become ``np.log1p(image_values)``.
    ...
    2. **Draw the transposed intensity image**::
       ...
       The transpose places energy vertically and momentum horizontally.

    3. **Draw the requested guide lines**::

           ax.axhline(0.0, ...)
           ax.axvline(position, ...)

       The Fermi line and each guide use the shared ``guide_color``.
    ...
    ax : Optional[Axes], optional
        Existing axis for the image. If ``None``, the function creates a
        figure and axis.
    cmap : str, optional
    ...
    fermi_line : bool, optional
        If True, draw a horizontal guide line at zero energy. Default
        True.
    momentum_guides : Optional[Tuple[float, ...]], optional
        Momentum positions of vertical guide lines in 1/Angstrom.
    energy_guides : Optional[Tuple[float, ...]], optional
        Energy positions of horizontal guide lines in eV.
    guide_color : str, optional
        Color of the Fermi line and each guide line. Default
        ``"white"``.
    xlabel : str, optional
    ...
    -----
    The runtime type checks reject an intensity shape that disagrees
    with the two physical axes. Use :func:`plot_arpes_spectrum` when the
    data lives in an :class:`ArpesSpectrum` carrier.

    See Also
    --------
    plot_arpes_spectrum : Plot an ARPES intensity map from an
        ArpesSpectrum PyTree.
    spectrum_extent : Compute the imshow extent of a momentum and an
        energy axis.
    """
    momentum_position: float
    energy_position: float

    image_values: Float64[NDArray, "n_k n_e"] = np.asarray(
        intensity, dtype=np.float64
    )
    if log_counts:
    ...
    extent: Tuple[float, float, float, float] = spectrum_extent(
        momentum_axis, energy_axis
    )
    image: AxesImage = ax.imshow(
        image_values.T,
        origin="lower",
    ...
    if fermi_line:
        ax.axhline(0.0, color=guide_color, linewidth=0.9, alpha=0.8)
    for momentum_position in momentum_guides or ():
        ax.axvline(
            momentum_position,
            color=guide_color,
            linewidth=0.8,
    ...
    for energy_position in energy_guides or ():
        ax.axhline(
            energy_position,
            color=guide_color,
            linewidth=0.8,
