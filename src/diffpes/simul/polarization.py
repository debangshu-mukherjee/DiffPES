...
def build_polarization_vectors(
    ...
    polarization_vectors: Tuple[Float64[Array, " 3"], Float64[Array, " 3"]] = (
        e_s,
        e_p,
    )
...
def photon_wavevector(
    ...
    k_hat: Float64[Array, " 3"] = k / jnp.linalg.norm(k)
    return k_hat
...
def polarization_from_angles(
    ...
    e_s_complex: Complex128[Array, " 3"] = e_s.astype(jnp.complex128)
    e_p_complex: Complex128[Array, " 3"] = e_p.astype(jnp.complex128)
    if kind == "s":
        polarization: Complex128[Array, " 3"] = e_s_complex
    elif kind == "p":
        ...
        angle: Float64[Array, " "] = jnp.asarray(
            polarization_angle,
            dtype=jnp.float64,
        )
        polarization = (
...
def polarization_to_spherical(
    ...
    """
    epsilon_x: Complex128[Array, " "] = polarization[0]
    epsilon_y: Complex128[Array, " "] = polarization[1]
    epsilon_z: Complex128[Array, " "] = polarization[2]
    root_two: Float64[Array, " "] = jnp.sqrt(
        jnp.asarray(2.0, dtype=jnp.float64)
    )
    epsilon_minus: Complex128[Array, " "] = (
        epsilon_x - 1j * epsilon_y
    ) / root_two
    epsilon_plus: Complex128[Array, " "] = (
        -(epsilon_x + 1j * epsilon_y) / root_two
    )
    spherical: Complex128[Array, " 3"] = jnp.stack(
...
def detector_rotation(
    ...
    if slit not in ("H", "V"):
        ...
        raise ValueError(msg)
    x_axis: Float64[Array, " 3"] = jnp.asarray(
        [1.0, 0.0, 0.0],
        dtype=jnp.float64,
    )
    y_axis: Float64[Array, " 3"] = jnp.asarray(
        [0.0, 1.0, 0.0],
        dtype=jnp.float64,
    )
    if slit == "H":
        rotation_y_tx: Float64[Array, "3 3"] = rodrigues_rotation(y_axis, tx)
        rotation_x_ty: Float64[Array, "3 3"] = rodrigues_rotation(x_axis, ty)
        rotation: Float64[Array, "3 3"] = rotation_x_ty @ rotation_y_tx
    else:
        rotation_x_tx: Float64[Array, "3 3"] = rodrigues_rotation(x_axis, tx)
        rotation_y_ty: Float64[Array, "3 3"] = rodrigues_rotation(y_axis, ty)
...
def sample_azimuth_rotation(
    ...
    Returns
    -------
    sample_orientation : Float64[Array, "3 3"]
    ...
    zero: Float64[Array, ""] = jnp.zeros_like(azimuth)
    one: Float64[Array, ""] = jnp.ones_like(azimuth)
    sample_orientation: Float64[Array, "3 3"] = jnp.stack(
        (
...
def lab_polarization_to_sample(
    ...
    Returns
    -------
    polarization_sample : Complex128[Array, " 3"]
...
def rotate_frame_vectors(
    vector: Float64[Array, " 3"],
    tx: Float64[Array, " n_tx"],
    ty: Float64[Array, " n_ty"],
    slit: str,
    sample_azimuth: ScalarFloat = 0.0,
) -> Float64[Array, "n_tx n_ty 3"]:
    """Rotate a detector-fixed real axis across a detector-angle grid.
    ...
    Returns
    -------
    rotated : Float64[Array, "n_tx n_ty 3"]
    ...
    def rotate_one_tx(
        ...
        Returns
        -------
        rotated_row : Float64[Array, "n_ty 3"]
        ...
        def rotate_one_ty(
            ...
            Returns
            -------
            rotated_vector : Float64[Array, " 3"]
            ...
            rotated_vector: Float64[Array, " 3"] = detector_axis_to_sample(
                vector,
                rotation,
                sample_orientation,
            )
        ...
        rotated_row: Float64[Array, "n_ty 3"] = jax.vmap(rotate_one_ty)(ty)
        return rotated_row

    rotated: Float64[Array, "n_tx n_ty 3"] = jax.vmap(rotate_one_tx)(tx)
    return rotated
